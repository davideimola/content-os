...
### Claude app capture door

The Claude app is the second capture door onto the Pipeline — the sibling of the terminal `contentos idea create`. From the Claude app on any device (phone or desktop), Davide dictates or types a raw Idea and it lands as an `idea`-labeled issue on `davideimola/content-os`, indistinguishable from one filed via the terminal door: `[Idea] ` + first-line summary title, spark verbatim as body, only the `idea` label. It is not an app of its own (ADR-0002) — the door is a Claude Project whose GitHub connector files the issue, steered by committed capture instructions that encode the same shape the Go door enforces (`internal/idea`); keep the two in step. No format/channel/quality decision at capture time (capture first, judge later; the Monday planning Beat judges it). Verified at the tracker seam — drive it and assert the resulting issue's shape — plus a phone live smoke test; no unit tests for the prompt. See `docs/agents/claude-app-capture.md`.
